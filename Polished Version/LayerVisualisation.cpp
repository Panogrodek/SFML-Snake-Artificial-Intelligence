...
void LayerVisualisation::InitLayers()
{
	for (int i = 0; i < m_layers.size(); i++) {
		sf::Vector2f startPos(0.67f * WINDOW_WIDTH, 0.5f * WINDOW_HEIGHT - (m_layers.at(i).GetSize() * 1.25f * (m_radius + i * 5)));
		std::vector<sf::CircleShape> vec;
		for (int j = 0; j < m_layers.at(i).GetSize(); j++) {
			sf::CircleShape shape;
			shape.setFillColor(sf::Color::White);
			shape.setRadius(m_radius + i * 5.f);
			shape.setPosition(sf::Vector2f(startPos.x + i * 200.f, startPos.y + j * 2.5f * (m_radius + i * 5.f)));
			vec.push_back(shape);
		}
